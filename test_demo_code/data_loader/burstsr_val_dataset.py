...
class BurstSRDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {'LR_burst': burst, 'HR': frame_gt, 'mask': mask, 'warp_matrix': warp_matrix, 'meta_info_burst': meta_info_burst, 'meta_info_gt': meta_info_gt}
...
if __name__ == "__main__":
    ...
    train_dataset = BurstSRDataset(root=root, 
    ...
    for epoch in range(1):
        ...
        for i, data in enumerate(trainset_loader):
            y, x, mask, warp_matrix, meta_info_y, meta_info_x  = data['LR_burst'], data['HR'], data['mask'], data['warp_matrix'], data['meta_info_burst'], data['meta_info_gt']
            #y = y * 255.
