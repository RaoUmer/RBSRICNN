...
from data_loader.burstsr_val_dataset import BurstSRDataset
...
test_img_folder = 'track2_val_set/' # testset LR burst images path
out_dir = 'sr_results_track2_val_set/' # save images path
os.makedirs(out_dir, exist_ok=True)
upscale_factor = 4 # upscaling factor
burst_size = 14 # number of burst size
...
# loading test data
val_dataset = BurstSRDataset(root=test_img_folder, burst_size=burst_size, split='val')
...
for i, data in enumerate(valset_loader):
    burst, mask, warp_matrix, meta_info_burst  = data['LR_burst'], data['mask'], data['warp_matrix'], data['meta_info_burst']
    burst_name = meta_info_burst['burst_name']
